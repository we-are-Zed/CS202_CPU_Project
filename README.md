...
4.在最初跑仿真时发现部分模块reset无效，后发现是部分模块复位信号部分高电平有效部分模块低电平有效，统一后重新进行仿真
...
同时对硬件和软件的结合的认识更深，在进行debug时，往往仿真的结果正确，轮到上板就发现要不没有输出要不显示不完全，往往是极其微小的地方出现错误，如汇编中寄存器自增时加错寄存器，复位信号忘记修改。这种debug难度很大，因为很难通过板上的显示进行debug，只能对着所有与bug相关连线的代码一条一条看。

另一个问题是应该早日开始这个项目，在中期答辩的13周我们才刚刚确立好分工，14周也是在各写各的代码，只有15周大家在一起合代码，因此最后一周没少熬夜赶进度，直到ddl的最后一天才成功检测完两个测试场景以及bonus部分，而其他开始的很早的组完成了五级流水线以及小游戏，得亏合作顺利，否则可能ddl之前无法完成所有检测。
...
### bonus问题
  1.VGA的显示经常缺少数据，比如应该与七段数码管一样有八位的输出，但VGA只显示高六位，经过检查，发现是VGA的边界设置过小，导致低二位信号没有被覆盖导致无输出

### 总结
bonus部分增加了部分指令以及实现了VGA。与硬件打交道是一个很麻烦的事情，你不知道到底哪里出错了，因此必须沉下心去看每一行代码，仔细检查其中是否有逻辑问题。
 ### 项目报告到此结束，感谢！
